...
<div align="center">
	<form id="Form"  autocomplete="off" enctype="multipart/form-data" method="post" name="Form" style="height: 100%;">
		<?php include('./common/common_mainmenu.php'); ?>
					...
					<table width="100%"><tbody><tr><td height="20" class="heading" >Application Search Result</td></tr></tbody></table>
					...
					<tr>
						...
						<th class="formHeader">Check ID</th>
                        ...
                        <th class="formHeader" style="width: 100px">Clearing Status</th>
						</tr></thead>
	<?php
           ...
           // echo($amount);
			...
			include('./connection.php');
             		...
             		$select_query="select * from pdcheck_details where reference_id='".$reference_id."' and paymentype like '%".$payType."%' 
...
and DATE(presentationdate) between '".$fdate."' and '".$tdate."' order by presentationdate desc;";
		...
		//echo($select_record);
			...
			while ($row = @mysql_fetch_assoc($select_record) ) {
						      print '<tr>'
	       .'<td><input type="radio" name="myradio" value="'.$row['check_id'].'" id="'. $row['check_id'].'"
	        onclick="EnableRadioPayment(this)"></td>'
	       .'<td>'. $row['check_id'] .'</td>'		
			.'<td>'. $row['paymentype'] .'</td>'				
			.'<td>'. $row['amount'] .'</td>'
            .'<td>'. $row['presentationdate'] .'</td>'
            .'<td>'. $row['presentationstatus'] .'</td>'
             .'<td>'. $row['clearingstatus'] .'</td>';
...
<div id="pageNavPosition" align="center"> </div>
			...
			<input name="btnBack" type="button" value="Back" onclick="location.href='searchStatus.php'" /></td> -->
                           ...
                           value='Back' onclick="setaction('searchStatus.php');" ></input>
                                                                  
			               <input type='submit' name="btnView" disabled="disabled"
                           value='View Application'  onclick="setaction('ViewPDCheckDetail.php');"></input> 
                           
                          
                       
                           <input type='submit' name="btnModify" disabled="disabled" 
                           value='Modify Application'  onclick="setaction('EditPDCheckDetail.php');"></input>
